...
neib :: [a] -> Int -> Int -> [a]
neib xs i lim = drop lower . take upper $ xs
...
dig2char :: (Eq a, Num a, Enum a, Show a) => a -> Char
...
set xs i x = take i xs ++ [dig2char x] ++ drop (i + 1) xs
...
solve (xs, lim) holes hindex
  ...
  where
    p = holes !! hindex
    tests = [solve (set xs p v, lim) holes (hindex + 1) | v <- [0 .. lim], fit (xs, lim) p v]
